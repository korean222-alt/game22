...
export function doorway(m, x, z, ry, w, h, col, openAng) {
  ...
  m.mat = MAT.WOOD;
  for (const k of [-1, 1]) {
    const ox = Math.cos(ry) * k * (w / 2 + 0.16), oz = -Math.sin(ry) * k * (w / 2 + 0.16);
    m.boxY(x + ox, h / 2, z + oz, 0.34, h, 0.34, ry, P.doorDk);
  }
  m.boxY(x, h + 0.18, z, 0.34, 0.36, w + 0.7, ry, P.doorDk);
  if (openAng !== undefined) {
    const a = ry + openAng;
    const hx = x + Math.cos(ry) * (w / 2), hz = z - Math.sin(ry) * (w / 2);
    m.boxY(hx - Math.cos(a) * (w / 2), h / 2, hz + Math.sin(a) * (w / 2), 0.16, h - 0.2, w, a, col || P.door);
  }
  m.mat = 0; m.flag = prev;
}
...
export function deskRect(m, x, z, ry, w, d, col) {
  ...
  // modesty panel
  const px = x - Math.cos(ry) * (d / 2 - 0.2), pz = z + Math.sin(ry) * (d / 2 - 0.2);
  m.mat = MAT.WOOD;
  m.boxY(px, 1.35, pz, 0.14, 1.5, w - 0.5, ry, shade(c, 0.92));
  m.mat = 0;
}
...
export function workstation(m, x, z, ry, variant, dual) {
  deskRect(m, x, z, ry, 6.4, 3.2, P.lam);
  const R = (lx, lz) => [x + lx * Math.cos(ry) + lz * Math.sin(ry), z - lx * Math.sin(ry) + lz * Math.cos(ry)];
  const back = R(0, -1.05);
  if (dual) {
    const a = R(-1.5, -1.05), b = R(1.5, -1.05);
    monitor(m, a[0], a[1], ry + 0.16, 2.2);
    monitor(m, b[0], b[1], ry - 0.16, 2.2);
  } else {
    monitor(m, back[0], back[1], ry, 2.6);
  }
...
export function stairs(m, x, z, ry, steps, run, rise) {
  m.mat = MAT.TILE;
  const n = steps || 18, r = run || 0.9, h = rise || (STOREY / (steps || 18));
  for (let i = 0; i < n; i++) {
    const d = (i + 0.5) * r;
    m.boxY(x + Math.cos(ry) * d, (i + 0.5) * h, z - Math.sin(ry) * d, 5.0, h, r, ry, P.slab);
  }
  m.mat = MAT.METAL;
  for (const k of [-1, 1]) {
    const ox = Math.sin(ry) * k * 2.6, oz = Math.cos(ry) * k * 2.6;
    for (let i = 0; i < n; i += 3) {
      const d = (i + 0.5) * r;
      m.boxY(x + Math.cos(ry) * d + ox, (i + 0.5) * h + 1.8, z - Math.sin(ry) * d + oz,
