...
export class UI {
  ...
  panelCompany(box) {
    ...
    const wk = el('button', 'btn primary wide', '다음 주로 (스태미나 · 체력 회복)');
    ...
    const walk = el('button', 'btn wide sm' + (this.view.walk ? ' primary' : ''),
      this.view.walk ? '1인칭 나가기' : '사무실을 직접 걸어보기 (F)');
    walk.onclick = () => { this.view.toggleWalk(); this.renderShell(); this.renderPanel(); };
