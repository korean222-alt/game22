...
/* 파는 동안에는 다음 게임을 시작할 수 없다. 화면은 막지 않는다 — 기획서도
   뽑고 직원도 키운다 — 그러나 **착수**만은 정산을 확인한 뒤다. 출시가 아무
   무게도 없는 사건이 되면 판매 곡선을 보는 사람이 아무도 없다. */
await step('판매 중에는 다음 게임에 착수할 수 없다', async () => {
  const r = await page.evaluate(() => {
    const g = window.__game;
    ...
    const btn = [...document.querySelectorAll('#panel button')].find((b) => b.textContent.includes('판매 중'));
    return {
      ok: res.ok, why: res.why, selling: !!g.sales,
      project: !!g.project, proposals: g.proposals.length, btn: btn ? btn.disabled : null,
    };
  });
  if (!r.selling) throw new Error('판매가 이미 끝났다 — 검사가 성립하지 않는다');
  if (r.ok || r.project) throw new Error('판매 중인데 착수가 됐다');
  if (!r.proposals) throw new Error('막혔는데 기획서가 사라졌다');
  return r.why;
});
